  ...
  useEffect(() => {
    const fun = async () => {
      ...
      if (treatmentname !== "" && doctorname === "") {
        ...
        const filtereddata = jsondata?.filter(
          (items) =>
            items?.DoctorName?.toLowerCase().trim() ===
        ...
        const filtereddata = jsondata?.filter(
          (items) =>
            items?.DoctorName?.toLowerCase()
              .trim()
              .includes(search?.toLowerCase().trim()) ||
            items?.Treatment?.toLowerCase().includes(search.toLowerCase()) ||
            items?.Time.includes(search) ||
            items?.Date.includes(search) ||
            items?.PatientName?.toLowerCase()
              .trim()
              .includes(search?.toLowerCase().trim())
        );
        setappoints(filtereddata);
